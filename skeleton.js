...
NeuroSarathi.UI.Skeleton = {
    ...
    _createJournalSkeleton: function() {
        ...
        div.style.cssText = `background: rgba(255,255,255,0.6); border-radius: 12px; padding: 14px 16px; border: 1px solid rgba(0,0,0,0.04); margin-bottom: 8px;`;
        div.innerHTML = `
            <div class="skeleton-line shimmer" style="height:12px;background:rgba(226,232,240,0.6);border-radius:8px;width:40%;margin-bottom:8px;"></div>
            <div class="skeleton-line shimmer" style="height:14px;background:rgba(226,232,240,0.6);border-radius:8px;width:80%;margin-bottom:6px;"></div>
            <div class="skeleton-line shimmer" style="height:12px;background:rgba(226,232,240,0.6);border-radius:8px;width:50%;"></div>
        `;
        return div;
    },
    ...
    _createProfileSkeleton: function() {
        ...
        div.style.cssText = `display: flex; align-items: center; gap: 16px; padding: 16px; background: rgba(255,255,255,0.6); border-radius: 16px; border: 1px solid rgba(0,0,0,0.04);`;
        div.innerHTML = `
            <div class="skeleton-img shimmer" style="width:56px;height:56px;border-radius:50%;background:rgba(226,232,240,0.6);flex-shrink:0;"></div>
            <div style="flex:1;">
                <div class="skeleton-line shimmer" style="height:16px;background:rgba(226,232,240,0.6);border-radius:8px;width:60%;margin-bottom:6px;"></div>
                <div class="skeleton-line shimmer" style="height:12px;background:rgba(226,232,240,0.6);border-radius:8px;width:40%;"></div>
            </div>
        `;
        return div;
    ...
    _createStatsSkeleton: function() {
        ...
        div.style.cssText = `display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px;`;
        div.innerHTML = `
            ${[1,2,3,4].map(() => `
                <div style="background:rgba(255,255,255,0.6);border-radius:12px;padding:16px;text-align:center;border:1px solid rgba(0,0,0,0.04);">
                    <div class="skeleton-line shimmer" style="height:24px;background:rgba(226,232,240,0.6);border-radius:8px;width:60%;margin:0 auto 6px;"></div>
...
function showSkeleton(containerId, count = 3, type = 'card') {
    NeuroSarathi.UI.Skeleton.show(containerId, count, type);
}
function hideSkeleton(containerId) {
    NeuroSarathi.UI.Skeleton.hide(containerId);
}
