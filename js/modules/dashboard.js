...
NeuroSarathi.Dashboard = {
    ...
    init: async function() {
        ...
        try {
            // FIX (v2): use getSession() instead of getUser() for the initial
            // auth check. getUser() makes a live network round-trip to Supabase's
            // server to re-validate the token every time — unnecessary here, and
            // fragile against slow connections, blockers, or transient network
            // issues, which was causing real logged-in users to be bounced back
            // to the login page immediately after a successful login.
            // getSession() reads the already-persisted session from local
            // storage directly — fast, reliable, no network dependency for this
            // check.
            const rawKeys = Object.keys(localStorage).filter(k => k.startsWith('sb-'));
            const rawValue = rawKeys.length > 0 ? localStorage.getItem(rawKeys[0]) : null;
            const session = await NeuroSarathi.Auth.getSession();
            alert('DEBUG 3 — location: ' + window.location.href +
                  ' | raw localStorage sb- keys: ' + JSON.stringify(rawKeys) +
                  ' | raw key has value: ' + (rawValue ? 'YES (' + rawValue.length + ' chars)' : 'NO') +
                  ' | getSession() result: ' + (session ? 'exists, user=' + session.user?.email : 'NULL'));
            ...
            try {
                ...
                alert('DEBUG 2 — profile loaded OK: onboarding_completed=' + profile.onboarding_completed);
            } catch (profileErr) {
                alert('DEBUG 2 — getProfile FAILED: ' + profileErr.message);
                throw profileErr;
            }
            ...
            alert('DEBUG 2 — init() CRASHED: ' + error.message + ' | stack: ' + (error.stack || '').substring(0, 300));
